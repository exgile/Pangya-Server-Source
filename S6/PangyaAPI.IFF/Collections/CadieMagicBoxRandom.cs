        ...
        public bool Load(MemoryStream data)
        {
            ...
            try
            {
                using (var Reader = new PangyaBinaryReader(data))
                {
                    ...
                    var datacount = Tools.IFFTools.SizeStruct(new CadieMagicBoxRandom());
                    if (datacount != recordLength)
                    {
                        throw new Exception($"CadieMagicBoxRandom.iff the structure size is incorrect, Real: {recordLength} CadieMagicBoxRandom.cs: {datacount} ");
                    }
            ...
            {
                MessageBox.Show(ex.Message + " Error Struct ", "Pangya.IFF.Model.CadieMagicBoxRandom");
                return false;
            }
        }
