        ...
        public bool Load(MemoryStream data)
        {
            ...
            try
            {
                using (var Reader = new PangyaBinaryReader(data))
                {
                    ...
                    var datacount = Tools.IFFTools.SizeStruct(new CutinInformation());
                    if (datacount != recordLength)
                    {
                        throw new Exception($"CutinInformation.iff the structure size is incorrect, Real: {recordLength} CutinInformation.cs: {datacount} ");
                    }
            ...
            {
                MessageBox.Show(ex.Message + " Error Struct ", "Pangya.IFF.Model.CutinInformation");
                return false;
            }
        }
