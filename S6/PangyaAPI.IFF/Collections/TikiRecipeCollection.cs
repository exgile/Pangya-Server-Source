        ...
        public bool Load(MemoryStream data)
        {
            ...
            try
            {
                using (var Reader = new PangyaBinaryReader(data))
                {
                    ...
                    var datacount = Tools.IFFTools.SizeStruct(new TikiRecipe());
                    if (datacount != recordLength)
                    {
                        throw new Exception($"TikiRecipe.iff the structure size is incorrect, Real: {recordLength} TikiRecipe.cs: {datacount} ");
                    }
            ...
            {
                MessageBox.Show(ex.Message + " Error Struct ", "Pangya.IFF.Model.TikiRecipe");
                return false;
            }
        }
