...
namespace PangyaAPI.IFF.Collections
{
    public class MatchCollection : List<Match>
    {
        ...
        public bool Load(MemoryStream data)
        {
            ...
            {
                MessageBox.Show(" data\\Match.iff is not loaded", "Pangya.IFF");
                return false;
            }
            ...
            try
            {
                using (var Reader = new PangyaBinaryReader(data))
                {
                    ...
                    var datacount = Tools.IFFTools.SizeStruct(new Match());
                    if (datacount != recordLength)
                    {
                        throw new Exception($"Match.iff the structure size is incorrect, Real: {recordLength} Match.cs: {datacount} ");
                    }

                    for (int i = 0; i < IFF_FILE_HEADER.RecordCount; i++)
                    {
                        Match = (Match)Reader.Read(new Match());
            ...
            {
                MessageBox.Show(ex.Message + " Error Struct ", "Pangya.IFF.Model.Match");
                return false;
            }
        }
