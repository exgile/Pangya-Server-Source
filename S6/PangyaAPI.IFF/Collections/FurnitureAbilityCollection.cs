        ...
        public bool Load(MemoryStream data)
        {
            ...
            try
            {
                using (var Reader = new PangyaBinaryReader(data))
                {
                    ...
                    var datacount = Tools.IFFTools.SizeStruct(new FurnitureAbility());
                    if (datacount != recordLength)
                    {
                        throw new Exception($"FurnitureAbility.iff the structure size is incorrect, Real: {recordLength} FurnitureAbility.cs: {datacount} ");
                    }
            ...
            {
                MessageBox.Show(ex.Message + " Error Struct ", "Pangya.IFF.Model.FurnitureAbility");
                return false;
            }
        }
