        ...
        public bool Load(MemoryStream data)
        {
            ...
            try
            {
                using (var Reader = new PangyaBinaryReader(data))
                {
                    ...
                    var datacount = Tools.IFFTools.SizeStruct(new Caddie());
                    if (datacount != recordLength)
                    {
                        throw new Exception($"Caddie.iff the structure size is incorrect, Real: {recordLength} Caddie.cs: {datacount} ");
                    }
            ...
            {
                MessageBox.Show(ex.Message + " Error Struct ", "Pangya.IFF.Model.Caddie");
                return false;
            }
        }
