        ...
        public bool Load(MemoryStream data)
        {
            ...
            try
            {
                using (var Reader = new PangyaBinaryReader(data))
                {
                    ...
                    var datacount = Tools.IFFTools.SizeStruct(new SetEffectTable());
                    if (datacount != recordLength)
                    {
                        throw new Exception($"SetEffectTable.iff the structure size is incorrect, Real: {recordLength} SetEffectTable.cs: {datacount} ");
                    }
            ...
            {
                MessageBox.Show(ex.Message + " Error Struct ", "Pangya.IFF.Model.SetEffectTable");
                return false;
            }
        }
