        ...
        public bool Load(MemoryStream data)
        {
            ...
            try
            {
                using (var Reader = new PangyaBinaryReader(data))
                {
                    ...
                    var datacount = Tools.IFFTools.SizeStruct(new ClubSet());
                    if (datacount != recordLength)
                    {
                        throw new Exception($"ClubSet.iff the structure size is incorrect, Real: {recordLength} ClubSet.cs: {datacount} ");
                    }
            ...
            {
                MessageBox.Show(ex.Message + " Error Struct ", "Pangya.IFF.Model.ClubSet");
                return false;
            }
        }
