...
using System.Text;
namespace PangyaAPI.IFF.BinaryModels
{
    public class PangyaBinaryWriter : BinaryWriter
    {
        ...
        public bool WriteStr(string message, int length)
        {
            ...
            try
            {
                ...
                Encoding.GetEncoding("Windows-1252").GetBytes(message).Take(length).ToArray().CopyTo(ret, 0);
        ...
        public bool WritePStr(string data)
        {
            ...
            try
            {
                var encoded = Encoding.GetEncoding("Windows-1252").GetBytes(data);
